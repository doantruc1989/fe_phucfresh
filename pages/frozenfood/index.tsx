...
function Index() {
  ...
  useEffect(() => {
    try {
      axios
        .get(
          `https://quocson2.fatcatweb.top/product?page=${page}&take=20&filter=5&sortField=${sortField2?.sortField}&fromPrice=${sortField2?.fromPrice}&toPrice=${sortField2?.toPrice}&search=${sortField2?.search}`
        )
        .then((res: any) => {
